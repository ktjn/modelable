...
| 2. Declaration/projection unification | Partial | `ResolvedDeclarationView` now covers model, projection, semantic-type, and enum-projection resolution while preserving legacy result properties; the IR still has parallel declaration families and specialized lookup paths remain. |
