...
| 2. Declaration/projection unification | Partial | `ResolvedDeclarationView` now covers model, projection, semantic-type, and enum-projection resolution; semantic-type and enum-projection version selection share one helper while legacy result properties and specialized namespace precedence remain. |
