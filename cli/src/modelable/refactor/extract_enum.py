...
# Every implemented codegen target (see emitters/targets.py), run against the
# candidate workspace before anything is written, so a target that would
# fail to generate from the extracted shape aborts the extraction instead of
# being discovered later (instruction #3).
_TARGET_EMITTERS: dict[str, object] = {
...
def _validate_target_outputs(workspace: Workspace) -> None:
    ...
    for target_name, emitter in _TARGET_EMITTERS.items():
        try:
