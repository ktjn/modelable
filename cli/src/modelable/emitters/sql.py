...
def _emit_projection_ddl(
    ...
    *,
    ...
    if dialect == "postgres":
        key_columns = _projection_key_columns(version, mdl, overlay)
        ...
        constraints.extend(_foreign_key_constraints(version, mdl, overlay))
        lines.append(f"CREATE TABLE IF NOT EXISTS {table_name} (")
        lines.append(",\n".join([*columns, *checks, *constraints]))
        lines.append(");")
        index_lines, index_warnings = _emit_secondary_index_ddl(version, table_name, mdl, overlay)
        lines.extend(index_lines)
        warnings.extend(index_warnings)
    else:
        index_clauses, index_warnings = _emit_clickhouse_secondary_index_clauses(version, mdl, overlay)
        warnings.extend(index_warnings)
        lines.append(f"CREATE TABLE IF NOT EXISTS {table_name}")
        lines.append("(")
...
def _resolve_table_name(
    ...
    if overlay is not None and source is not None:
        source_domain, source_model = version.source.model.rsplit(".", 1)
        identity = f"{source_domain}.{source_model}@{source.version}"
        table = overlay.resolve(identity).values.get("table")
...
def _resolve_column_name(
    ...
    projection_name: str,
    mdl: MdlFile,
    overlay: OverlayDocument | None,
) -> str:
    default = _snake_case(field.name)
    if overlay is None:
        return default
    source = _resolve_source_model(version, mdl)
    if isinstance(field.mapping, DirectMapping) and source is not None:
        source_domain, source_model = version.source.model.rsplit(".", 1)
        identity = f"{source_domain}.{source_model}@{source.version}"
        path = f"{identity}#{field.mapping.source_field}"
    else:
        identity = f"{domain_name}.{projection_name}@{version.version}"
        path = f"{identity}#{field.name}"
    column = overlay.resolve(identity, path).values.get("column")
    return column if isinstance(column, str) else default


def _validate_sql_overlay(overlay: OverlayDocument, dialect: str) -> None:
    expected_target = f"sql-{dialect}"
    if overlay.target != expected_target:
        raise OverlayError(f"overlay target {overlay.target!r} does not match {expected_target!r}")
    for entry in overlay.entries:
        expected_key = "table" if entry.section == "models" else "column"
        unknown = sorted(set(entry.values) - {expected_key})
        if unknown:
            raise OverlayError(
                f"{entry.selector!r} has unsupported {expected_target} overlay key(s): {', '.join(unknown)}"
            )
        value = entry.values.get(expected_key)
        if not isinstance(value, str) or not _SQL_IDENTIFIER.fullmatch(value):
            raise OverlayError(f"{entry.selector!r} {expected_key} must be a SQL identifier")
    for key, value in overlay.defaults.items():
        if key not in {"table", "column"}:
            raise OverlayError(f"unsupported {expected_target} overlay default: {key!r}")
        if not isinstance(value, str) or not _SQL_IDENTIFIER.fullmatch(value):
            raise OverlayError(f"overlay default {key!r} must be a SQL identifier")
...
def _emit_secondary_index_ddl(
    ...
    for secondary in index_decl.secondary:
        ...
        for field_name in secondary.key:
            column = _resolve_projection_column(field_name, version, mdl, overlay)
            if column is None:
        ...
        for sort_field in secondary.sort:
            column = _resolve_projection_column(sort_field.field, version, mdl, overlay)
            if column is None:
                warnings.append(
...
def _emit_clickhouse_secondary_index_clauses(
    version: ProjectionVersion,
    mdl: MdlFile,
    overlay: OverlayDocument | None = None,
) -> tuple[list[str], list[str]]:
...
def _resolve_projection_column(
    ...
    for field in version.fields:
        if isinstance(field.mapping, DirectMapping) and field.mapping.source_field == source_field_name:
            if overlay is not None and mdl is not None:
                source = _resolve_source_model(version, mdl)
                if source is not None:
                    source_domain, source_model = version.source.model.rsplit(".", 1)
                    identity = f"{source_domain}.{source_model}@{source.version}"
                    configured = overlay.resolve(identity, f"{identity}#{source_field_name}").values.get("column")
...
def _projection_key_columns(
    ...
    return [
        ...
        if (column := _resolve_projection_column(field, version, mdl, overlay)) is not None
    ]
...
def _foreign_key_constraints(
    ...
    for field in source.fields:
        if not isinstance(field.type, RefType):
            continue
        column = _resolve_projection_column(field.name, version, mdl, overlay)
        target = _resolve_ref_target(field.type, mdl, overlay)
        if column is None or target is None:
            continue
        target_table, target_column = target
        constraints.append(f"    FOREIGN KEY ({column}) REFERENCES {target_table} ({target_column})")
    return constraints
...
def _resolve_ref_target(
    ...
    if overlay is not None:
        configured = overlay.resolve(f"{target_domain}.{target_model}@{target_version.version}").values.get("table")
        if isinstance(configured, str):
            table = configured
        if key is not None:
            target_identity = f"{target_domain}.{target_model}@{target_version.version}"
            configured_key = overlay.resolve(target_identity, f"{target_identity}#{key}").values.get("column")
            if isinstance(configured_key, str):
                return table, configured_key
