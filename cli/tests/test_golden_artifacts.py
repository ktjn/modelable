...
from modelable.emitters.base import EmittedArtifact, compute_content_hash, render_artifact_text
...
def _assert_artifact_structurally_valid(target: str, artifact: EmittedArtifact, rendered: str) -> None:
    path = Path(artifact.path)
    suffix = path.suffix.lower()
    if suffix in {".json", ".avsc"}:
...
""",
    ...
    for target, emitter in generator.TARGET_EMITTERS.items():
        ...
        for artifact in artifacts:
            assert artifact.content_hash == compute_content_hash(artifact.content)
            _assert_artifact_structurally_valid(target, artifact, render_artifact_text(artifact))
