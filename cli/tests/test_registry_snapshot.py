...
def test_registry_diff_reports_sql_target_compatibility_for_changed_artifact(tmp_path: Path) -> None:
    source = tmp_path / "models.mdl"
    source.write_text(FIXTURE.read_text(encoding="utf-8"), encoding="utf-8")
    output_dir = tmp_path / ".modelable"
    old_manifest = {
        "target": {"name": "sql-postgres"},
        "artifacts": [
            {
                "path": "customer.sql",
                "ref": "customer.Customer@1",
                "sha256": "a" * 64,
                "content": 'CREATE TABLE "customer" ("id" UUID);',
            }
        ],
    }
    new_manifest = {
        ...
        "artifacts": [
            {
                ...
                "ref": "customer.Customer@1",
                "sha256": "b" * 64,
                "content": 'CREATE TABLE "customer" ("id" UUID, "name" TEXT);',
            }
        ],
    }
    ...
    assert any(
        consequence["action"] == "storage_migration" and consequence["status"] == "migration_required"
...
def test_registry_diff_reports_openapi_target_compatibility_for_changed_artifact(tmp_path: Path) -> None:
    source = tmp_path / "models.mdl"
    source.write_text(FIXTURE.read_text(encoding="utf-8"), encoding="utf-8")
    output_dir = tmp_path / ".modelable"
    old_content = {
        "openapi": "3.1.0",
        "paths": {"/customers/{id}": {"get": {"responses": {"200": {"description": "ok"}}}}},
    }
    new_content = {"openapi": "3.1.0", "paths": {}}
    old_manifest = {
        "target": {"name": "openapi"},
        "artifacts": [
            {
