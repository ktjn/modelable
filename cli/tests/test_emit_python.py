...
def test_emit_python_pure_value_type_does_not_import_other_domains(tmp_path):
...
""",
    ...
    artifact = next(item for item in emit_python(workspace, tmp_path / "out") if item.ref == "pure.Tag@1")
    assert "from patient." not in artifact.content


def test_emit_python_cross_domain_semantic_ref_emits_inline_primitive(tmp_path):
    source = tmp_path / "model.mdl"
    source.write_text(
        """domain patient {
  owner: "patient-team"
...
""",
    ...
    assert "patientId: UUID" in artifact.content
    assert "PatientPatientId" not in artifact.content


def test_emit_python_cross_domain_value_ref_adds_one_import(tmp_path):
    source = tmp_path / "model.mdl"
    source.write_text(
        """domain patient {
  owner: "patient-team"
