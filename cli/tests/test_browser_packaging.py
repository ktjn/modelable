...
def test_browser_module_selection_excludes_desktop_surfaces() -> None:
    ...
    assert "modelable/facets.py" in selected
...
def test_browser_build_backend_is_exactly_constrained() -> None:
    ...
    assert pyproject["build-system"]["requires"] == ["hatchling==1.32.3"]
    assert (browser_root / "build-constraints.txt").read_text(encoding="utf-8") == ("hatchling==1.32.3\n")
