...
3. **CLI-owned sync.** `modellable compile` resolves the graph, fetches foreign models, compiles, and writes back consumer registrations. No manual sync step.
...
A registry node is a **git repository** containing `.mdl` source files, a `workspace.mdl` that declares its identity and peer edges, and a `.modellable/` build output directory.
...
The CLI owns all sync. The developer only runs `modellable compile`.
...
5. **Sync peers in dependency order** — for each peer: `git fetch <remote>`, sparse-checkout `.mdl` files into `.modellable/mirror/<peer-id>/`.
...
.modellable/                         # build artifacts — gitignore everything except noted
...
Everything under `.modellable/` is a build artifact and can be regenerated by running `modellable compile`.
...
`registry.db` and `mirror/` are fully rebuildable by running `modellable compile` from:
...
```bash
modellable compile           # full cycle
modellable compile --dry-run # show what would be synced and written back
```
...
modellable registry graph    # print the DAG with sync state
modellable registry sync     # force-sync all peers regardless of sync mode
...
| :--- | :--- |
| Registry node disk failure | Rebuild `registry.db` and `mirror/` by running `modellable compile`. All source of truth is in git. |
...
| `consumers/` directory deleted | Re-generated on the next `modellable compile` by write-back from downstream registries. Historical entries recoverable from git history. |
| Split-brain (two nodes claim to own the same domain) | `modellable compile` detects the conflict during graph resolution and aborts. |
...
4. Run `modellable compile`. The CLI fetches mirrors and writes consumer entries.
...
4. Run `modellable compile` on each workspace to wire up mirrors and consumer write-backs.
...
```
modellable compile  (analytics-registry)
...
The customer team now sees `consumers/analytics-registry/CustomerOrderSummary@1.mdl` in their own repo via the merged PR. When they plan `customer.Customer@4 (breaking)`, `modellable dependents customer.Customer@3` lists the analytics projection, and CI can gate the breaking version on the analytics team updating their pinned import.
