...
- [ ] Rebuild the derived registry index from local source plus the exact lock
  and objects, with no implicit refresh.
- [ ] Add resolve/verify/status fixtures for missing objects, hash mismatch,
...
- [ ] Show exact dependency, generated-artifact, and required-action changes.
