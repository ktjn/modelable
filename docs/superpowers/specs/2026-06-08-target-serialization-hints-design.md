...
1. **Where do generated `#[serde(with = "…")]` helper modules live?**
   - **Decision:** **Option (b) (Inline helper modules in generated output).**
   - **Reasoning:** Keeps Modelable completely dependency-free for consumers, matching the "no runtime materialization" Phase 1 posture. The emitter will inline the source of the necessary helper modules into the generated Rust artifacts.
2. **Should a hint change be flagged by compatibility checks?**
   - **Decision:** **Yes.** A hint change directly alters the generated artifact's wire shape. While not a breaking change to the canonical model itself, it is a breaking change for the generated artifact. It will be flagged as an "artifact compatibility" warning by `modelable diff`.
3. **Is `@manual` needed at MVP?**
   - **Decision:** **No, defer it.** We will start without `@manual`. For domains with complex or unsupported transforms, the adopting project can exclude them from codegen via their project configuration rather than adding speculative IDL-level constructs.
4. **`typescript.*` modifiers:**
   - **Decision:** Defer to the JSON Schema emitter change. Modifiers like `@wire(json: "string")` will compile to JSON Schema format extensions, which `json-schema-to-typescript` inherently translates to `string` in TypeScript without requiring custom TS-emitter casing.
