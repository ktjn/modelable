...
- OpenLineage projection emission now consumes validated `modelable.plan/v1` documents.
...
- SQL projection emission now consumes validated `modelable.plan/v1` documents.

- `registry usage` can aggregate validated compiled-consumer manifests across applications.
...
- Migrate event-sink projection emission to the stable `modelable.plan/v1` contract.
...
- The C# emitter now consumes normalized `modelable.plan/v1` documents through
  its parser-free projection renderer.

- The Java emitter now consumes normalized `modelable.plan/v1` documents through
  its parser-free projection renderer.

- The Go emitter now consumes normalized `modelable.plan/v1` documents through
  its parser-free projection renderer.
...
- Registry lock requirements now preserve the selected object's source
  provenance and verify it alongside the resolved signature and content hash.
...
- Plain Rust projection fields now consume validated `modelable.plan/v0` documents
  while preserving existing generated records.
...
- `compile` can now consume a validated local registry snapshot with
  `--snapshot` for offline external-contract references.
...
- Registry locks now record deterministic requested-to-resolved dependency
  ...
  conflicting content for one logical identity. Enum snapshot objects now
  retain the same source provenance as model and projection objects.
